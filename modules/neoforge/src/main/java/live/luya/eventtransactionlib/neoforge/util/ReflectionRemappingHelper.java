...
import live.luya.eventtransactionlib.neoforge.EventTransactionLibMod;
...
public class ReflectionRemappingHelper {
	...
	public BiFunction<MappingType, String, List<String>> namingFunction() {
		return this::findMapping;
	}

	private List<String> findMapping(MappingType domain, String srgName) {
		switch (domain) {
			case MappingType.FIELD -> {
				return this.findFieldMapping(srgName);
			}
			case MappingType.METHOD -> {
